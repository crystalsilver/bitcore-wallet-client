...
Utils.signDigibyteMessage = function(text, path, xprv) {
  var hdPrivateKey = Bitcore.HDPrivateKey(xprv);
  var derivedHdPrivateKey = hdPrivateKey.derive("m/44'/0'/0'" + path);
  var derivedPrivateKey = derivedHdPrivateKey.privateKey;
  var signature = Message(text).sign(derivedPrivateKey);
